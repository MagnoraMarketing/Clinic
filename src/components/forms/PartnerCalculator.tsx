  ...
  return (
    <div className="grid gap-6 rounded-[32px] border border-white/8 bg-ink-900/80 p-6 sm:p-8 lg:grid-cols-[1fr_1fr]">
      <div className="space-y-6">
        <h2 className="h-display text-3xl">{t("Earnings calculator")}</h2>
        <label className="block">
          <span className="label">{t("New clinics per month")} · {perMonth}</span>
          <input type="range" min={1} max={20} value={perMonth} onChange={(e) => setPerMonth(Number(e.target.value))} className="w-full accent-[#3fcfab]" />
        </label>
        <label className="block">
          <span className="label">{t("Period · {n} months", { n: months })}</span>
          <input type="range" min={3} max={24} value={months} onChange={(e) => setMonths(Number(e.target.value))} className="w-full accent-[#3fcfab]" />
        </label>
        <div>
          <span className="label">{t("Typical AI package")}</span>
          <div className="grid grid-cols-3 gap-2">
            {PRICES.ai.map((p) => (
              <button key={p.key} type="button" onClick={() => setPkg(p.key)} className={`rounded-2xl border px-3 py-2.5 text-start text-xs transition ${pkg === p.key ? "border-sage-400 bg-sage-400/10" : "border-white/10 hover:border-white/25"}`}>
                <span className="block font-semibold">{t(p.name.replace("AI minutes · ", ""))}</span>
                {eur(p.price)} / {t(p.unit ?? "")}
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="grid gap-3 rounded-3xl bg-gradient-to-br from-sage-400/15 to-ink-950 p-6">
        <Row label={t("{n} clinics signed", { n: clinics })} value="" />
        <Row label={t("One-off (50 % of website + import)")} value={eur(Math.round(oneOff))} />
        <Row label={t("Recurring over the period")} value={eur(Math.round(recurring))} />
        <div className="mt-2 border-t border-white/10 pt-4">
          <p className="text-xs text-ink-400">{t("Total over {n} months", { n: months })}</p>
          <p className="h-display text-5xl text-sage-300">{eur(Math.round(oneOff + recurring))}</p>
          <p className="mt-2 text-sm text-ink-300">{t("…and {amount} per month in recurring income after month {n}.", { amount: eur(Math.round(monthlyAtEnd)), n: months })}</p>
        </div>
      </div>
    </div>
  );
