...
export function AdminShell({ demoMode, storage, children }: { demoMode: boolean; storage: string; children: React.ReactNode }) {
  ...
  const switcher = (
    <label className="block">
      <span className="mb-1.5 block text-[10px] font-bold tracking-wider text-ink-400 uppercase">Clinic (tenant)</span>
        ...
        <div className="mt-auto space-y-3">
          {clinic && (
            <Link href={`/demo/${clinic.slug}`} target="_blank" className="flex items-center gap-2 rounded-xl border border-white/10 px-3 py-2.5 text-xs text-ink-300 hover:text-white">
              <Icon name="external" className="h-4 w-4" /> Open {clinic.name}&apos;s website
            </Link>
          )}
          <p className="rounded-xl bg-white/5 px-3 py-2 text-[11px] text-ink-400">
            {demoMode ? "🧪 Demo mode · no login" : "🔒 Protected admin"} · storage: {storage}
          </p>
          {!demoMode && (
            <button
              onClick={async () => {
      ...
      <div className="min-w-0">
        ...
        {demoMode && (
          <div className="border-b border-sage-500/20 bg-sage-500/10 px-4 py-2 text-center text-xs text-sage-200 sm:px-8 print:hidden">
            Public admin demo – data is fictional and resets regularly. Book via{" "}
            <Link href={`/demo/${clinic?.slug ?? ""}`} className="font-semibold underline">
              the clinic&apos;s website
            </Link>{" "}
            or the AI receptionist and watch it appear here.
          </div>
        )}
  ...
  return (
    <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
      <div>
        ...
        {text && <p className="mt-1 max-w-3xl text-sm text-ink-400">{text}</p>}
      </div>
