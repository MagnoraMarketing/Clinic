  ...
  return (
    <div className={`overflow-hidden rounded-[22px] border border-white/10 bg-ink-900 shadow-2xl shadow-black/60 ${className}`}>
      <div className="flex items-center gap-2 border-b border-white/8 bg-ink-850 px-4 py-2.5">
        ...
        <span className="ms-3 flex-1 truncate rounded-full bg-ink-950 px-3 py-1 text-center text-[11px] text-ink-400">🔒 {url}</span>
      </div>
      {children}
    </div>
  );
    ...
    <div className="bg-ink-950">
      ...
      <div className="relative h-52 sm:h-64">
        ...
        <div className="absolute bottom-4 left-5">
          ...
          <p className="text-sm text-white/75">{t(clinic.tagline)}</p>
          <div className="mt-3 flex gap-2 text-xs font-semibold">
            <span className="rounded-full px-3 py-1.5 text-ink-950" style={{ background: accent }}>
              📅 {t("Book online")}
            </span>
            <span className="rounded-full bg-white/10 px-3 py-1.5">{t("🔄 Move booking")}</span>
            <span className="hidden rounded-full bg-white/10 px-3 py-1.5 sm:inline">{t("📞 Call")}</span>
          </div>
        </div>
      </div>
      <div className="grid grid-cols-3 gap-3 p-4">
        {featured.map((s) => (
          <div key={s.id} className="rounded-xl bg-ink-900 p-3 ring-1 ring-white/8">
            ...
            <p className="mt-1 truncate text-[11px] font-semibold">{t(s.name)}</p>
    ...
    <BrowserFrame url="your-clinic.com" className="relative">
      <div className="relative h-[380px] bg-gradient-to-br from-ink-900 to-ink-950 p-6">
        ...
        <div className="absolute right-5 bottom-5 w-72 rounded-3xl border border-white/10 bg-ink-900 p-5 shadow-2xl">
          <p className="text-sm font-semibold">{t("🎙️ Talk to us")}</p>
          <p className="text-xs text-ink-400">{t("The AI receptionist is listening…")}</p>
          <Waveform className="mt-4" />
          <p className="mt-3 rounded-2xl bg-ink-800 px-3 py-2 text-xs text-white/85">{t("“Can I move my haircut to Saturday morning?”")}</p>
          <p className="mt-2 rounded-2xl bg-sage-400 px-3 py-2 text-xs text-ink-950">{t("“Done! You're now booked Saturday at 10:00 with Ida ✓”")}</p>
  ...
  return (
    <div className="card p-5 sm:p-6">
      <div className="flex items-center justify-between">
        <p className="font-semibold">{t("Thursday · Calm Hands")}</p>
        <span className="rounded-full bg-white/5 px-3 py-1 text-xs">{t("3 therapists · 15 min buffer")}</span>
      </div>
          ...
          <div key={s.name} className="relative rounded-xl bg-white/[0.03]" style={{ height: hours.length * H }}>
            ...
            {blocks
              ...
              .map((b) => (
                ...
                >
                  <span className="block truncate font-semibold">{t(b.label)}</span>
                  {b.ai && <span className="block truncate text-[9.5px] opacity-80">{t("🎙️ booked by AI")}</span>}
                  {b.moved && <span className="block truncate text-[9.5px] opacity-80">{t("🔄 moved by AI")}</span>}
                </div>
              ))}
          </div>
        ))}
      ...
      <div className="mt-5 flex items-center gap-3 rounded-2xl border border-emerald-400/25 bg-emerald-400/10 p-3 text-sm">
        ...
        <span>
          <strong>{t("Booked by phone")}</strong> – deep tissue with Sofie 10:15–11:15 · ref. CA-4218
        </span>
      </div>
    </div>
  );
  ...
  return (
    <div className="relative grid items-center gap-6 sm:grid-cols-2">
      <div className="mx-auto w-[270px] rounded-[40px] border-[9px] border-ink-800 bg-ink-950 p-4 shadow-2xl">
        ...
        <p className="text-center text-[11px] text-ink-400">{t("SMS · Studio Nord Hair")}</p>
        <div className="mt-3 space-y-2 text-[12px]">
          <p className="w-fit max-w-[90%] rounded-2xl rounded-es-md bg-ink-800 px-3 py-2">{t("You're booked: Women's cut with Ida, Thu 11:00. Ref. ST-4211.")}</p>
          <p className="ms-auto w-fit max-w-[90%] rounded-2xl rounded-ee-md bg-sage-400 px-3 py-2 text-ink-950">{t("Can I move it to Saturday morning?")}</p>
          <p className="w-fit max-w-[90%] rounded-2xl rounded-es-md bg-ink-800 px-3 py-2">{t("Ida is free Sat 10:00 or 11:15. Which one?")}</p>
          <p className="ms-auto w-fit rounded-2xl rounded-ee-md bg-sage-400 px-3 py-2 text-ink-950">10:00 👍</p>
          <p className="w-fit max-w-[90%] rounded-2xl rounded-es-md bg-ink-800 px-3 py-2">{t("Moved ✓ Sat 10:00 with Ida.")}</p>
        </div>
      </div>
      <div className="space-y-3">
        <div className="card p-4">
          <p className="text-xs font-semibold tracking-wide text-ink-400 uppercase">{t("Change log · ST-4211")}</p>
          <div className="mt-3 flex items-center gap-3 text-sm">
            <span className="rounded-xl bg-white/5 px-3 py-2 text-ink-400 line-through">{t("Thu 11:00")}</span>
            <Icon name="arrow" className="h-4 w-4 text-sage-400" />
            <span className="rounded-xl bg-sage-400/15 px-3 py-2 font-semibold text-sage-200">{t("Sat 10:00")}</span>
          </div>
          <p className="mt-2 text-xs text-ink-400">{t("🎙️ by AI voice · 2 min ago · Thu 11:00 is free again")}</p>
        </div>
        <div className="card p-4">
          <p className="text-xs font-semibold tracking-wide text-ink-400 uppercase">{t("Cancellation · CA-4209")}</p>
          <p className="mt-2 text-sm">{t("“I'm ill and can't come tomorrow.”")}</p>
          <p className="mt-2 rounded-xl bg-amber-400/10 px-3 py-2 text-xs text-amber-200">{t("Less than 24 h before → AI explains the DKK 300 fee and offers to move it instead.")}</p>
        </div>
      </div>
    </div>
  );
...
export function PricesVisual({ catalog }: { catalog: Catalog }) {
  ...
  return (
    <div className="relative">
      <div className="card p-5 sm:p-6">
        <p className="font-semibold">{t("Price list · synced to the AI")}</p>
      ...
      <div className="absolute -right-2 -bottom-8 w-72 rounded-3xl border border-white/10 bg-ink-900 p-4 shadow-2xl sm:-right-6">
        <p className="rounded-2xl rounded-es-md bg-ink-800 px-3 py-2 text-xs">{t("“How much is a deep tissue massage – and does my insurance cover it?”")}</p>
        <p className="mt-2 rounded-2xl rounded-ee-md bg-sage-400 px-3 py-2 text-xs text-ink-950">
          {t("“60 minutes is DKK 695. It's covered by Sygeforsikringen danmark groups 1, 2 and 5 – shall I find you a time?”")}
        </p>
      </div>
    </div>
  );
    ...
    <div className="relative grid items-end gap-6 sm:grid-cols-2">
      <div className="relative mx-auto w-60">
        <div className="rounded-t-3xl rounded-b-lg bg-cream p-5 text-ink-950 shadow-2xl">
          ...
          <p className="text-center text-[11px] text-ink-600">{t("Scan to book your next visit")}</p>
      ...
      <div className="mx-auto w-56 rounded-[36px] border-[8px] border-ink-800 bg-ink-950 p-3 shadow-2xl">
        ...
        <p className="text-xs font-semibold">{t("Google reviews")}</p>
        <p className="mt-1 font-display text-3xl font-semibold">4.9 ★</p>
        <p className="text-[10px] text-ink-400">{t("312 reviews · +46 this month via NFC")}</p>
        {["Best massage in Østerbro!", "Booked by phone at 22:00 – the AI sorted it.", "Lovely, calm clinic."].map((r) => (
          <p key={r} className="mt-2 rounded-xl bg-ink-900 px-2 py-2 text-[11px]">
            ⭐⭐⭐⭐⭐ <span className="block text-ink-300">{t(r)}</span>
          </p>
        ))}
      </div>
        ...
        <div className="space-y-3 p-4">
          ...
          <div className="rounded-xl bg-ink-850 p-3">
            <div className="mb-2 flex items-center justify-between">
              <p className="font-semibold">{t("Live call")}</p>
            ...
            <p className="mt-2 text-ink-300">{t("“…could I move my Thursday appointment to Friday morning?”")}</p>
          </div>
    ...
    <div className="grid items-center gap-6 lg:grid-cols-[1fr_auto_1fr_auto_1fr]">
      ...
      <div className="relative mx-auto grid h-56 w-56 place-items-center rounded-full bg-gradient-to-br from-sage-300 to-sage-700 text-center text-ink-950 shadow-2xl shadow-sage-600/30">
        ...
        <div>
          ...
          <p className="text-xs text-ink-950/70">{t("understands · checks · books")}</p>
        </div>
      </div>
      <Arrow />
      <div className="grid gap-2">
        ...
        <div className="flex items-center gap-3 rounded-2xl border border-emerald-400/30 bg-emerald-400/10 px-4 py-3 text-sm font-semibold text-emerald-200">{t("🗓️ Straight into your calendar / system")}</div>
      </div>
    </div>
  );
}
