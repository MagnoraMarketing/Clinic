            ...
            <div className="flex rounded-full bg-white/5 p-1 text-sm">
              {(["day", "list"] as const).map((v) => (
                <button key={v} onClick={() => setView(v)} className={`rounded-full px-4 py-1.5 ${view === v ? "bg-white text-ink-950" : "text-ink-300"}`}>
                  {v === "day" ? "Day calendar" : "Upcoming list"}
                </button>
              ))}
            </div>
      ...
      {view === "day" ? (
        <>
          <div className="mb-5 flex flex-wrap items-center gap-3">
            <button onClick={() => setDate(addDays(date, -1))} className="btn-secondary !px-3 !py-2" aria-label="Previous day">
              ←
            </button>
            <p className="min-w-56 text-center font-semibold">{formatDate(date)}</p>
            <button onClick={() => setDate(addDays(date, 1))} className="btn-secondary !px-3 !py-2" aria-label="Next day">
              →
            </button>
            ...
            <span className="ml-auto text-sm text-ink-400">
              {active.length} appointments · {dkk(active.reduce((s, a) => s + a.price, 0))}
            </span>
          </div>

          {!hours ? (
            <p className="card p-10 text-center text-ink-400">The clinic is closed on this day.</p>
          ) : (
              ...
              <div className="grid min-w-[720px] gap-2" style={{ gridTemplateColumns: `48px repeat(${staff.length}, minmax(160px, 1fr))` }}>
                ...
                {staff.map((p) => (
                  <div key={p.id} className="flex items-center gap-2 pb-2 text-sm font-semibold">
                    ...
                    {!p.workDays.includes(day as never) && <span className="text-[10px] font-normal text-ink-400">off</span>}
                  </div>
                ))}
                    ...
                    {list
                      ...
                      .map((a) => (
                        <button
                          ...
                          className={`absolute inset-x-1 overflow-hidden rounded-lg px-2 py-1 text-left text-[11px] leading-tight transition hover:brightness-125 ${selected === a.id ? "ring-2 ring-white" : ""} ${a.status === "pending" ? "border border-dashed border-amber-300" : ""}`}
                        ...
                        >
                          ...
                          <span className="block truncate opacity-80">
                            {SOURCE[a.source].icon} {a.serviceName}
                            {a.changes.length ? " · 🔄" : ""}
                          </span>
                        </button>
                      ))}
          ...
          {list.some((a) => a.date === date && a.status === "cancelled") && (
            <p className="mt-3 text-xs text-ink-400">
              ...
              {list
                ...
                .map((a) => `${a.time} ${a.customer.name}${a.lateCancellation ? " (late)" : ""}`)
                .join(" · ")}
            </p>
          )}
        </>
        ...
        <>
          ...
          <div className="card divide-y divide-white/5">
            {list.map((a) => (
              <button key={a.id} onClick={() => setSelected(a.id)} className={`flex w-full flex-wrap items-center gap-4 p-4 text-left hover:bg-white/[0.02] ${a.status === "cancelled" ? "opacity-50" : ""}`}>
                <span className="w-28 text-sm">
                  <span className="block font-semibold tabular-nums">{a.time}</span>
                  <span className="text-xs text-ink-400">{formatDate(a.date).split(" ").slice(0, 3).join(" ")}</span>
                </span>
                <span className="min-w-40 flex-1">
                  <span className="block font-semibold">{a.customer.name}</span>
                  <span className="text-xs text-ink-400">
                    {a.serviceName} · {a.practitionerName} · {SOURCE[a.source].icon} {SOURCE[a.source].label}
                    {a.changes.length ? ` · 🔄 moved ${a.changes.length}×` : ""}
                  </span>
                </span>
                <span className={`rounded-full px-2.5 py-1 text-[11px] font-bold ${STATUS[a.status].cls}`}>{STATUS[a.status].label}</span>
              </button>
            ))}
            {list.length === 0 && <p className="p-10 text-center text-ink-400">No upcoming appointments.</p>}
          </div>
        </>
      )}
...
function Drawer({ a, onClose, onPatch, onMoved }: { a: Appointment; onClose: () => void; onPatch: (a: Appointment, body: Record<string, unknown>) => Promise<Appointment>; onMoved: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black/50 backdrop-blur-sm" onClick={onClose}>
      <aside className="h-full w-full max-w-md animate-slide-in overflow-y-auto border-l border-white/10 bg-ink-900 p-6" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-xs text-ink-400">{a.reference}</p>
            <h2 className="h-display text-2xl">{a.customer.name}</h2>
          </div>
          <button onClick={onClose} className="text-ink-400 hover:text-white" aria-label="Close">
            ✕
          </button>
        </div>
        <span className={`mt-3 inline-block rounded-full px-2.5 py-1 text-[11px] font-bold ${STATUS[a.status].cls}`}>{STATUS[a.status].label}</span>
        {a.lateCancellation && <span className="ml-2 rounded-full bg-amber-400/15 px-2.5 py-1 text-[11px] font-bold text-amber-300">Late cancellation</span>}
        <dl className="mt-5 space-y-2 text-sm">
          {[
            ["Treatment", a.serviceName],
            ["When", `${formatDate(a.date)} · ${a.time}`],
            ["Duration", `${a.durationMinutes} min`],
            ["With", a.practitionerName],
            ["Price", dkk(a.price)],
            ["Phone", a.customer.phone],
            ["Email", a.customer.email ?? "–"],
            ["Booked via", `${SOURCE[a.source].icon} ${SOURCE[a.source].label}`],
            ["New client", a.newClient ? "Yes" : "No"],
          ].map(([k, v]) => (
        ...
        {(a.status === "confirmed" || a.status === "pending") && (
          <div className="mt-6 rounded-2xl border border-white/8 p-4">
            ...
            {moveDate && (
              <div className="mt-3">
                {slots === null ? (
                  <p className="text-xs text-ink-400">Finding free times…</p>
                ) : slots.length === 0 ? (
                  <p className="text-xs text-ink-400">No free times that day.</p>
                ) : (
        ...
        {a.changes.length > 0 && (
          <div className="mt-6">
            <p className="text-xs font-semibold tracking-wide text-ink-400 uppercase">Change log</p>
              ...
              {a.changes.map((c, i) => (
                <li key={i} className="rounded-xl bg-ink-850 px-3 py-2">
                  ...
                  <span className="block text-xs text-ink-400">
                    {SOURCE[c.by]?.icon} {SOURCE[c.by]?.label} · {formatDateTime(c.at)}
                  </span>
                </li>
              ))}
            </ul>
