...
export default function QrNfcPage() {
  ...
  const saveReview = async () => {
    ...
    try {
      ...
      await refreshClinic();
      setMsg("Saved ✓ The NFC chip now sends clients to the new link – no need to change the chip.");
    } catch (e) {
    ...
    <>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <section className="card space-y-4 p-6">
          <h2 className="font-semibold">🔳 Booking QR code</h2>
          <label className="block">
            <span className="label">Open booking with a treatment pre-selected (optional)</span>
          ...
          <div className="flex flex-wrap items-center gap-6">
            <div className="rounded-2xl bg-cream p-4 text-ink-950 shadow-xl">
              ...
              <p className="text-center text-[10px]">Scan to book</p>
        ...
        <section className="card space-y-4 p-6">
          <h2 className="font-semibold">⭐ NFC review chip</h2>
          <p className="text-sm text-ink-400">The chip is programmed with a fixed address. You decide where it points – e.g. your Google review page – and can change it anytime.</p>
          <p className="break-all rounded-xl bg-ink-850 px-3 py-2 font-mono text-xs">
            Chip address: {origin}
            {nfcPath}
          </p>
          <label className="block">
            <span className="label">Review link (https://)</span>
