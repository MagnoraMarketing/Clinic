  ...
  return (
    <>
        ...
        <div className="container-x">
          <div className="mx-auto max-w-4xl animate-fade-up text-center">
            <span className="eyebrow">
              <span className="h-1.5 w-1.5 rounded-full bg-sage-400" /> For massage, hair, chiro, physio, dental, skin & foot clinics
            </span>
            <h1 className="h-display mt-6 text-[2.5rem] leading-[1.05] sm:text-6xl lg:text-7xl">
              Your clinic answers <span className="bg-gradient-to-r from-sage-200 via-sage-400 to-sand-400 bg-clip-text text-transparent italic">every call</span> – even when you&apos;re with a client
            </h1>
            <p className="mx-auto mt-6 max-w-2xl text-lg text-ink-300">
              AIbooking is the AI receptionist on your phone line and your website. It books, rebooks and cancels appointments, explains prices and insurance – straight into your calendar.
            </p>
            <div className="mt-8 flex flex-wrap justify-center gap-3">
              <a href="#demo" className="btn-primary !px-6 !py-3.5 text-base">
                Try the AI receptionist <Icon name="arrow" className="h-4 w-4" />
              </a>
              <a href="#phone" className="btn-secondary !px-6 !py-3.5 text-base">
                <Icon name="phone" className="h-4 w-4" /> Hear a phone call
              </a>
            </div>
          </div>
          ...
          <div id="demo" className="relative mt-14 scroll-mt-24 animate-fade-up [animation-delay:150ms]">
            ...
            <p className="mb-3 flex flex-wrap items-center justify-center gap-2 text-center text-xs text-ink-400">
              <span className="h-2 w-2 animate-pulse rounded-full bg-emerald-400" /> Example: {clinic.name}&apos;s website with AIbooking – try the receptionist on the right
            </p>
            ...
            <div className="mt-6 flex flex-wrap justify-center gap-3 text-sm">
              <Link href={`/demo/${slug}`} className="btn-secondary !py-2.5">
                Open the full clinic website
              </Link>
              <Link href="/admin/appointments" className="btn-ghost !py-2.5">
                See bookings land in the calendar →
              </Link>
            </div>
          </div>
        </div>
        ...
        <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {CALL_TYPES.map((c) => (
            <div key={c.title} className="card p-6">
              ...
              <OpenReceptionistButton message={c.example.replace(/[“”]/g, "")} className="mt-4 w-full rounded-2xl bg-ink-850 px-3 py-2.5 text-left text-sm text-white/85 transition hover:bg-ink-800">
                {c.example} <span className="text-sage-400">→</span>
      ...
      <section id="solutions" className="border-t border-white/8 bg-gradient-to-b from-ink-900/60 to-ink-950 pt-20 sm:pt-28">
        <SectionHead eyebrow="Solutions" title="What we offer your clinic" text="Start with the AI on your phone, add the voice widget to your website – or get a complete clinic website with booking. Everything works together." />
      </section>

      <Feature o={offering("phone")} index={1} visual={<ConversationDemo />}>
        <div className="mt-6 flex flex-wrap items-center gap-4 rounded-2xl border border-white/10 bg-ink-900 p-4">
          ...
          <div className="flex-1">
            <p className="text-xs text-ink-400">Call the demo line</p>
      ...
      <Feature o={offering("voice-widget")} index={2} flip visual={<VoiceWidgetVisual />}>
        <div className="mt-6 flex flex-wrap gap-3">
          <OpenReceptionistButton voice className="btn-primary">
            <Icon name="mic" className="h-4 w-4" /> Try the voice widget
          </OpenReceptionistButton>
      ...
      <Feature o={offering("booking")} index={3} visual={<BookingVisual />}>
        <div className="mt-6 flex flex-wrap gap-3">
      ...
      <Feature o={offering("rebooking")} index={4} flip visual={<RebookingVisual />}>
        <div className="mt-6 flex flex-wrap gap-3">
          <OpenReceptionistButton message="I need to move my appointment" className="btn-primary">
            <Icon name="refresh" className="h-4 w-4" /> Move a booking with AI
          </OpenReceptionistButton>
      ...
      <Feature o={offering("prices")} index={5} visual={<PricesVisual catalog={catalog} />}>
        <div className="mt-6 flex flex-wrap gap-3">
          <OpenReceptionistButton message="How much is a deep tissue massage?" className="btn-primary">
            Ask about a price
          </OpenReceptionistButton>
          <OpenReceptionistButton message="Does my insurance cover massage?" className="btn-secondary">
            Ask about insurance
          </OpenReceptionistButton>
        </div>
      </Feature>

      <Feature o={offering("reviews")} index={6} flip visual={<ReviewsVisual slug={slug} name={clinic.name} />}>
        <div className="mt-6 flex flex-wrap gap-3">
          <Link href="/admin/qr-nfc" className="btn-primary">
            QR & NFC in the backend
          </Link>
      ...
      <Feature o={offering("website")} index={7} visual={<WebsitesVisual clinics={clinics} />}>
        <div className="mt-6 flex flex-wrap gap-3">
      ...
      <section className="container-x py-20 sm:py-28">
        <div className="grid items-center gap-12 lg:grid-cols-[0.8fr_1.2fr]">
          <div>
            ...
            <h2 className="h-display mt-5 text-4xl sm:text-5xl">Every call and every booking in one calm overview</h2>
            <p className="mt-4 text-lg text-ink-300">See calls live, read what the AI agreed with the client, and manage appointments, practitioners, prices and integrations – per clinic.</p>
      ...
      <section id="pricing" className="scroll-mt-20 border-y border-white/8 bg-ink-900/40 py-20 sm:py-28">
        <div className="container-x">
          <SectionHead eyebrow="Pricing" title="Simple pricing for clinics of every size" text="A clinic website and the platform – then add AI minutes for the phone and voice widget when you're ready." />
          <div className="mt-12">
            <Pricing />
          </div>
          <p className="mt-8 text-center text-sm text-ink-400">
            Questions about pricing or a package for several locations?{" "}
      ...
      <section id="clinics" className="container-x scroll-mt-20 py-20 sm:py-28">
        <SectionHead eyebrow="Clinic types" title="One receptionist – seven kinds of clinics" text="See how the AI receptionist handles the calls that are typical for your kind of clinic." />
        <div className="mt-12 grid grid-cols-2 gap-4 md:grid-cols-4 lg:grid-cols-7">
          {CLINIC_TYPES.map((v) => (
            <Link key={v.slug} href={`/clinics/${v.slug}`} className="group relative overflow-hidden rounded-3xl border border-white/8">
              <Photo src={v.image} alt={v.name} emoji={v.emoji} className="h-48 transition duration-500 group-hover:scale-105 sm:h-60" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/30 to-transparent" />
              <div className="absolute inset-x-0 bottom-0 p-4">
                <p className="font-semibold">
                  {v.emoji} {v.name}
                </p>
                <p className="mt-1 text-[11px] text-white/70">{v.pain}</p>
              </div>
            </Link>
          ))}
        </div>
          ...
          <ul className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {["Your own website", "Your own design", "Your treatments & prices", "Your own team", "Your own calendar", "Your cancellation policy", "Your own phone number", "Your own integrations"].map((x) => (
              <li key={x} className="flex items-center gap-3 rounded-2xl border border-white/8 bg-ink-900 px-4 py-3 text-sm">
                <Icon name="check" className="h-4 w-4 text-sage-400" /> {x}
              </li>
            ))}
          </ul>
        </div>
      ...
      <section id="integrations" className="container-x scroll-mt-20 py-20 sm:py-28">
        <SectionHead eyebrow="Integrations" title="Works with the system you already have" text="You don't have to switch booking or journal system to use AIbooking." />
        <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
          {INTEGRATIONS.map((i) => (
            <div key={i.title} className="card relative p-6">
              {i.tag && <span className="absolute top-4 right-4 rounded-full bg-sage-500/15 px-2 py-0.5 text-[10px] font-bold text-sage-300 uppercase">{i.tag}</span>}
        ...
        <div id="api" className="mt-10 grid scroll-mt-24 gap-6 rounded-[28px] border border-white/8 bg-ink-900/60 p-6 sm:p-8 lg:grid-cols-2">
          <div>
            <p className="text-sm font-semibold text-sage-300">API-first</p>
            <p className="h-display mt-2 text-2xl">Availability, bookings and rebookings via one REST API</p>
            <p className="mt-2 text-sm text-ink-400">Phone AI, voice widget, chat, website and your practice system all use the same endpoints – with signed webhooks for every event.</p>
          </div>
      ...
      <section className="bg-gradient-to-b from-ink-950 via-ink-900/60 to-ink-950 py-20 sm:py-28">
        <div className="container-x">
          <SectionHead eyebrow="Benefits" title="Less time on the phone. More time with clients." />
          <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
            {BENEFITS.map((b) => (
      ...
      <section className="container-x pb-24">
        <div className="relative overflow-hidden rounded-[40px] bg-gradient-to-br from-sage-300 via-sage-500 to-sage-700 p-10 text-center text-ink-950 sm:p-16">
          ...
          <h2 className="h-display relative mx-auto max-w-3xl text-4xl sm:text-5xl">See what AIbooking can do for your clinic</h2>
          <p className="relative mx-auto mt-4 max-w-xl text-lg text-ink-950/80">Try the AI receptionist right here – or book a demo and we&apos;ll set it up with your treatments, prices and team.</p>
...
function Feature({ o, index, flip, visual, children }: { o: Offering; index: number; flip?: boolean; visual: React.ReactNode; children?: React.ReactNode }) {
  return (
          ...
          <ul className="mt-6 grid gap-2 sm:grid-cols-2">
            {o.bullets.map((b) => (
              <li key={b} className="flex gap-2 text-sm text-ink-300">
                <Icon name="check" className="mt-0.5 h-4 w-4 shrink-0 text-sage-400" /> {b}
              </li>
            ))}
          </ul>
          {children}
